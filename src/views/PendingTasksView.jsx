...
import { formatDuration, formatTime, calculateTaskDuration, formatTaskTimeRange } from '../utils/dateUtils';
              ...
              >
                ...
                <div className="flex items-start gap-3.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {/* Metadata line: Course, Subject, Scheduled Date & Timing info */}
                    <div className="flex flex-wrap items-center gap-2 text-[11px] text-slate-400 mt-1.5">
                      ...
                      {task.originalDate && (
                        <span className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-100 dark:bg-white/[0.05] border border-slate-200/60 dark:border-white/10 text-slate-600 dark:text-slate-300 font-medium">
                          ...
                          Sch: {task.originalDate}
                        </span>
                      )}
                    </div>

                    {/* Auto-Logged Start Time, Completion Time & Time Taken Display */}
                    <div className="flex flex-wrap items-center gap-2 text-[11px] mt-2">
                      {task.startedAt && (
                        <span className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-sky-500/10 border border-sky-400/30 text-sky-600 dark:text-sky-400 font-bold">
                          <Clock className="w-3 h-3 text-sky-400" />
                          Start: {formatTime(task.startedAt)}
                        </span>
                      )}
                      {task.completedAt && (
                        <span className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-emerald-500/10 border border-emerald-400/30 text-emerald-600 dark:text-emerald-400 font-bold">
                          <CheckCircle2 className="w-3 h-3 text-emerald-400" />
                          End: {formatTime(task.completedAt)}
                        </span>
                      )}
                      {task.status === 'Completed' && (task.actualMinutes || (task.startedAt && task.completedAt)) && (
                        <span className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-amber-500/10 border border-amber-400/30 text-amber-600 dark:text-amber-400 font-extrabold font-mono">
                          ⏱️ Time Taken: {formatDuration(task.actualMinutes || calculateTaskDuration(task.startedAt, task.completedAt))}
                        </span>
                      )}
                      {!task.completedAt && task.estimatedMinutes && (
                        <span className="text-slate-400 font-medium">
                          ⏱️ Est: {task.estimatedMinutes}m
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  {task.status === 'Pending' || task.status === 'In Progress' ? (
                    <>
                      <button
                        ...
                        className={`py-2 px-4 rounded-xl text-white font-extrabold text-xs shadow-md flex items-center gap-1.5 transition-all hover:scale-105 active:scale-95 ${
                          task.status === 'In Progress'
                            ? 'bg-gradient-to-r from-amber-500 to-amber-400 shadow-amber-500/25 animate-pulse'
                            : 'bg-gradient-to-r from-sky-500 to-sky-400 shadow-sky-500/25'
                        }`}
                        title={task.status === 'In Progress' ? 'Continue Study Timer' : 'Start Task (Auto logs start time)'}
                      >
                        <Play className="w-3.5 h-3.5 fill-current" />
                        <span>{task.status === 'In Progress' ? '▶ In Progress' : '▶ Start Task'}</span>
                      </button>
                      ...
                      <button
                        ...
                        title="Mark Complete (Auto logs completion time & duration)"
                      >
